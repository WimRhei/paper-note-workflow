...
| poppler-windows | Provides Windows Poppler binaries, including `pdftotext.exe`, when no local `pdftotext` is found. | <https://github.com/oschwartz10612/poppler-windows> | The packaging repository is MIT licensed. Poppler itself is GPL licensed; this skill calls `pdftotext` as an external command and does not redistribute Poppler binaries in this repository or release assets. Keep upstream license files from the downloaded archive. |
